(() => {
  ...
  function renderRings() {
    ...
    while (r <= maxRadius) {
      ringLayer.appendChild(createSvg('circle', {
        cx: CENTER,
        cy: CENTER,
        r,
        fill: 'none',
  ...
  function addAimMarker(point) {
    ...
    g.appendChild(createSvg('line', {
      x1: point.x - 16, y1: point.y, x2: point.x + 16, y2: point.y,
      stroke: '#136f3a', 'stroke-width': 2
    }));
    g.appendChild(createSvg('line', {
      x1: point.x, y1: point.y - 16, x2: point.x, y2: point.y + 16,
      stroke: '#136f3a', 'stroke-width': 2
    }));
  ...
  function addPoibMarker(point) {
    const g = createSvg('g');
    g.appendChild(createSvg('line', {
      x1: point.x - 12, y1: point.y - 12, x2: point.x + 12, y2: point.y + 12,
  ...
  function addCorrectionArrow(from, to) {
    ...
    marker.appendChild(createSvg('path', { d: 'M 0 0 L 10 5 L 0 10 z', fill: '#0b57d0' }));
    defs.appendChild(marker);
    overlayLayer.appendChild(defs);
    overlayLayer.appendChild(createSvg('line', {
      x1: from.x, y1: from.y, x2: to.x, y2: to.y,
      stroke: '#0b57d0', 'stroke-width': 4, 'marker-end': 'url(#arrowHead)'
    }));
  }
  ...
  function directionWords(offsetXInches, offsetYInches) {
    const horizontal = offsetXInches > 0 ? 'right' : offsetXInches < 0 ? 'left' : 'centered';
    const vertical = offsetYInches > 0 ? 'low' : offsetYInches < 0 ? 'high' : 'centered';
  ...
  function renderSec(results) {
    const windageSummary = results.clickDirections.windage === 'NONE'
      ? 'No windage correction'
      : `${results.windageClicks} clicks ${results.clickDirections.windage}`;
    const elevationSummary = results.clickDirections.elevation === 'NONE'
  ...
  function showResults() {
    if (!state.aimPoint || state.shots.length < 3) {
      statusText.textContent = 'You need 1 aim point and at least 3 impacts before results can be shown.';
      return;
    }
    const results = calculateResults();
    redrawOverlay(true);
  ...
  function resetSimulator() {
    ...
    secCard.innerHTML = '<div class="sec-brand">Shooter Experience Card</div><div class="sec-empty">Results will appear after you set an aim point and tap your shots.</div>';
    redrawOverlay(false);
    setMode('aim');
  }
