...
(() => {
  ...
  const B2B_ZONES = [
    ...
    { id: 7,  shape: "circle", cx: 0.16, cy: 0.78, r: 0.105 },
    { id: 8,  shape: "square", cx: 0.50, cy: 0.78, hw: 0.105, hh: 0.105 },
    { id: 9,  shape: "circle", cx: 0.84, cy: 0.78, r: 0.105 },

    { id: 10, shape: "circle", cx: 0.50, cy: 0.94, r: 0.11 }
  ];
  ...
  function scoreB2BFromHits(hitPoints) {
    ...
    for (const hit of hitPoints) {
      ...
      for (const zone of B2B_ZONES) {
        if (pointInB2BZone(hit, zone)) {
          hitLanes.add(zone.id);
          matched = true;
        }
      }

      if (!matched) missedTaps += 1;
    }
